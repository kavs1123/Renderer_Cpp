...
Vector3f Scene::hemispheresampling() {
    float e1 = next_float();
    float e2 = next_float();

    float theta = acos(e1); // Theta
    float phi = 2 * M_PI * e2;       // Phi
    ...
    Vector3f newDir(x, y, z);

    return newDir;

}

Vector3f Scene::cosine_sampling(float e1, float e2) {
